execute as @a[scores={opt_preset=1..}] run function loumardes:scaffolding_rush/options/preset
execute if entity @a[scores={opt_build_height=1..}] run function loumardes:scaffolding_rush/options/build_height
execute if entity @a[scores={opt_gravel=1..}] run function loumardes:scaffolding_rush/options/gravel
execute if entity @a[scores={opt_instant_pil=1..}] run function loumardes:scaffolding_rush/options/instant_pillar
execute if entity @a[scores={opt_lava_speed=1..}] run function loumardes:scaffolding_rush/options/lava_speed
execute if entity @a[scores={opt_perf_mode=1..}] run function loumardes:scaffolding_rush/options/perf_mode
execute if entity @a[scores={opt_snowball=1..}] run function loumardes:scaffolding_rush/options/snowball
execute if entity @a[scores={opt_perf_mode=1..}] run function loumardes:scaffolding_rush/options/perf_mode
execute if entity @a[scores={opt_team_egg=1..}] run function loumardes:scaffolding_rush/options/team_egg
execute if entity @a[scores={opt_vil_resp_cd=1..}] run function loumardes:scaffolding_rush/options/vil_resp_cd
execute if entity @a[scores={opt_vil_respawn=1..}] run function loumardes:scaffolding_rush/options/vil_respawn
execute as @a[scores={opt_wb_size_add=1..}] run function loumardes:scaffolding_rush/options/wb_size_add
execute as @a[scores={opt_wb_size_add=..-1}] run function loumardes:scaffolding_rush/options/wb_size_add
execute as @a[scores={opt_randomteam=..-1}] run function loumardes:scaffolding_rush/options/random_team
