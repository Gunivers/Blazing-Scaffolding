...
execute if score PerformanceMode options matches 0 if score LavaCountdown global >= LavaSpeedTics options as @e[type=area_effect_cloud,name="ScR_LavaLevel"] at @s run function scaffolding_rush:lava/rise_globally
execute if score LavaCountdown global >= LavaSpeedTics options as @e[type=area_effect_cloud,name="ScR_Build"] at @s run function scaffolding_rush:game/air_replace
