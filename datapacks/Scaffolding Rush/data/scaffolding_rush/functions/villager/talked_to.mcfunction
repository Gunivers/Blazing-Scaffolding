...
execute unless score #game.loading data matches 1 \
        ...
        if entity @a[tag=!player.is_dead,advancements={scaffolding_rush:grab_villager/red=true},scores={player.pick_villager_cooldown=..0}] \
    run function utils:clean_kill

execute unless score #game.loading data matches 1 \
        unless score #villager.movable options matches 0 \
        as @e[type=villager,tag=blue_villager] \
        if entity @a[tag=!player.is_dead,advancements={scaffolding_rush:grab_villager/blue=true},scores={player.pick_villager_cooldown=..0}] \
    run function utils:clean_kill

execute unless score #game.loading data matches 1 \
        unless score #villager.movable options matches 0 \
        as @e[type=villager,tag=green_villager] \
        if entity @a[tag=!player.is_dead,advancements={scaffolding_rush:grab_villager/green=true},scores={player.pick_villager_cooldown=..0}] \
    run function utils:clean_kill

execute unless score #game.loading data matches 1 \
        unless score #villager.movable options matches 0 \
        as @e[type=villager,tag=yellow_villager] \
        if entity @a[tag=!player.is_dead,advancements={scaffolding_rush:grab_villager/yellow=true},scores={player.pick_villager_cooldown=..0}] \
    run function utils:clean_kill
