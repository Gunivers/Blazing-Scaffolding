...
execute unless score #game.loading data matches 1 \
    ...
    run function utils:clean_kill
execute unless score #villager.movable options matches 0 if entity @s[scores={player.pick_villager_cooldown=..0},tag=!Respawning] as @e[type=villager,tag=red_villager] if entity @a[advancements={scaffolding_rush:grab_villager/red=true}] run function utils:clean_kill
execute unless score #villager.movable options matches 0 if entity @s[scores={player.pick_villager_cooldown=..0},tag=!Respawning] as @e[type=villager,tag=yellow_villager] if entity @a[advancements={scaffolding_rush:grab_villager/yellow=true}] run function utils:clean_kill
