...
tellraw @a[tag=lobby.just_exit_protected_area,scores={option.language=0}] [{"text":"Welcome in the training playground!","color":"gold","bold":true},{"text":"\n\nYou can test here all the game mechanics. These mechanics are adapted to the selected preset/options.\n\nAlso, the worldborder and the bases' position reflect the game's ones.","color":"gray","bold":false},{"text":"\n---","color":"gold","bold":true}]
tellraw @a[tag=lobby.just_exit_protected_area,scores={option.language=1}] [{"text":"Bienvenue sur le terrain d'entrainement !","color":"gold","bold":true},{"text":"\n\nVous pouvez tester ici toutes les mécaniques de jeu. Ces mécaniques s'adaptent au(x) preset/options sélectionnées.\n\nDe plus, la bordure du monde ainsi que la position des bases reflètent celles de la prochaine partie.","color":"gray","bold":false},{"text":"\n---","color":"gold","bold":true}]
