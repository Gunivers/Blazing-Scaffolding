
#tag used to detect when the player should have the score enabled
tag @s add game_options_enabled

scoreboard players enable @s option.preset
scoreboard players enable @s option.lava_speed
scoreboard players enable @s option.build_height
scoreboard players enable @s option.sand
scoreboard players enable @s option.bumping_arrow
scoreboard players enable @s option.fireball
scoreboard players enable @s option.ender_pearl
scoreboard players enable @s option.vil_respawn
scoreboard players enable @s option.vil_resp_cd
scoreboard players enable @s option.instant_pil
scoreboard players enable @s option.perf_mode
scoreboard players enable @s option.team_egg
scoreboard players enable @s option.wb_size_add
scoreboard players enable @s option.regen
scoreboard players enable @s option.fall_damage
scoreboard players enable @s option.random_team
scoreboard players enable @s option.team_number
scoreboard players enable @s option.resp_delay
scoreboard players enable @s option.admin
scoreboard players enable @s option.villager
scoreboard players enable @s option.language
scoreboard players enable @s option.lasting_bases
scoreboard players enable @s option.flag_take_over
scoreboard players enable @s option.interactible_lobby
scoreboard players enable @s option.volcano
scoreboard players enable @s option.volcano_summon_period
scoreboard players enable @s option.volcano_pop_period
scoreboard players enable @s option.volcano_target_rate
scoreboard players enable @s option.scaff_stops_arrow

scoreboard players enable @s trigger.reset
scoreboard players enable @s trigger.start_game

#generated scores
scoreboard players enable @s option.time_limit
scoreboard players enable @s option.score_limit
scoreboard players enable @s option.use_time_limit
scoreboard players enable @s option.use_score_limit
scoreboard players enable @s option.worldborder_shrink
scoreboard players enable @s option.worldborder_start_time
scoreboard players enable @s option.worldborder_time_to_shrink
scoreboard players enable @s option.lava_speed
scoreboard players enable @s option.fast_climb
scoreboard players enable @s option.movable_villager
scoreboard players enable @s option.invulnerable_villager
