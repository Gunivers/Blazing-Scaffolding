...
execute unless score #villager.invulnerable options matches 0 if score #game.running data matches 1 as @e[type=villager,tag=respawn_villager] at @s if score @s pos.y <= @e[type=marker,name="lava.level",limit=1] pos.y run function scaffolding_rush:game/prevent_villager_from_falling_in_lava
